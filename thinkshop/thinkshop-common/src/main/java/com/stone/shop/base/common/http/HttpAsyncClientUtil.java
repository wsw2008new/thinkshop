package com.stone.shop.base.common.http;
...
import com.stone.shop.base.common.utils.MyX509TrustManager;

public class HttpAsyncClientUtil {
	public static final Logger logger = LoggerFactory.getLogger(HttpAsyncClientUtil.class);
	...
	static {
		try {
			...
			niocm = new PoolingClientAsyncConnectionManager(reactor);
		} catch (Exception e) {
			logger.error(e.getMessage(), HttpAsyncClientUtil.class);
		}
	}
	...
	public static void get(String url, FutureCallback<HttpResponse> callback) {
		try {
			logger
					.debug("Get [" + url + "] ...", HttpAsyncClientUtil.class);
	...
	public static void postXml(String url, String xml, String requestCharset,
			FutureCallback<HttpResponse> callback) {
		post(url, xml, "text/xml; charset=" + requestCharset, "text/xml",
	...
	public static void postJson(String url, String json, String requestCharset,
			FutureCallback<HttpResponse> callback) {
		post(url, json, "application/json; charset=" + requestCharset,
				"application/json", requestCharset, callback);
	}
	...
	public static void post(String url, Map<String, String> params,
		...
		logger.debug("Post [" + url + "] ...", HttpAsyncClientUtil.class);
		try {
			HttpAsyncClient client = new DefaultHttpAsyncClient(niocm);
			IOReactorStatus state = client.getStatus();
			if (!state.equals(IOReactorStatus.ACTIVE)) {
	...
	public static void sslGet(String url, Map<String, String> params,
		...
		try {
			AsyncScheme sch = getSslScheme(keyType, keyPath, keyPassword,
					sslport);
	...
	private static void sslGet(String url, Map<String, String> params,
			...
			FutureCallback<HttpResponse> callback) {
		logger
				.debug("SSL Get [" + url + "] ...", HttpAsyncClientUtil.class);
	...
	public static void sslPostXml(String url, String xml, String keyType,
		...
		try {
			AsyncScheme sch = getSslScheme(keyType, keyPath, keyPassword,
					sslPort);
			sslPost(url, xml, "text/xml; charset=" + DEFAULT_CHARSET,
					"text/xml", DEFAULT_CHARSET, sch, authString, callback);
		} catch (Exception e) {
			logger.debug(e.getMessage(), HttpAsyncClientUtil.class);
		}
	}
	...
	public static void sslPostJson(String url, String json, String keyType,
		...
		try {
			AsyncScheme sch = getSslScheme(keyType, keyPath, keyPassword,
					sslPort);
			sslPost(url, json, "application/json; charset=" + DEFAULT_CHARSET,
					"application/json", DEFAULT_CHARSET, sch, authString,
					callback);
		} catch (Exception e) {
			logger.debug(e.getMessage(), HttpAsyncClientUtil.class);
		}
